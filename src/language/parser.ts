...
// @ts-ignore
import { Lexer, createLexer } from './lexer.ts';
// @ts-ignore
...
class Parser {
  ...
  parseImplementsInterfaces(): Array<NamedTypeNode> {
    ...
    if (this.expectOptionalKeyword('implements')) {
      ...
      do {
        ...
        this.expectOptionalToken(TokenKind.AMP) || // Legacy support for the SDL?
   ...
   */
  expectOptionalToken(kind: TokenKindEnum): Token | null {
    const token = this._lexer.token;
    if (token.kind === kind) {
      this._lexer.advance();
      return token;
   ...
   * is encountered.
   */
  unexpected(atToken?: Token | null): GraphQLError {
