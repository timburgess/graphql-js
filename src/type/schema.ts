...
export class GraphQLSchema {
  ...
  __validationErrors: ReadonlyArray<GraphQLError> | null;
  ...
  constructor(config: GraphQLSchemaConfig): void {
    ...
    const initialTypes: Array<GraphQLNamedType | null> = [
      this._queryType,
      this._mutationType,
  ...
  getType(name: string): GraphQLNamedType | null {
  ...
  getDirective(name: string): GraphQLDirective | null {
...
export type GraphQLSchemaValidationOptions = {
   ...
   */
  allowedLegacyNames?: ReadonlyArray<string> | null;
};
...
function typeMapReducer(map: TypeMap, type: GraphQLType | null): TypeMap {
  if (!type) {
    return map;
  }
